...
* [`src/writenator/styles.cljs`](src/writenator/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/writenator/`](src/writenator/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/writenator/core.cljs): contains the SPA entry point, `init`
* [`test/writenator/`](test/writenator/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `writenator` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/writenator/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns writenator.example
  (:require [writenator.config :as config])
